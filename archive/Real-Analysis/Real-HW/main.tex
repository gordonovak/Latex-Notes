...
% Custom Integral Shortcuts
\newcommand{\dint}{\displaystyle\int}
\newcommand{\tint}{\textstyle\int}
\newcommand{\dinti}[2]{\displaystyle\int_{#1}^{#2}}
\newcommand{\tinti}[2]{\textstyle\int_{#1}^{#2}}

% General shortcuts (cases, inverses, fractions)
...
\newcommand{\purp}[1]{\textcolor{magenta}{#1}}
\newcommand{\viol}[1]{\textcolor{violet}{#1}}

% Epsilon shortcuts (because \Epsilon is too much for me)
...
\subsection*{Problem 1 (M)} \FGR (part d needs resubmitting)\\
\FGM\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection*{Problem 5 (M)} \FGR (both parts)\\
    \SGR (b)\\
    \FGM\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        \begin{proof}
            ...
            Hence, because $\forall x \in (0,2), x\in \bigcup_{n=1}^\infty \lbr \frac{1}{n}, 2\rpa$, $\bigcup_{n=1}^\infty \lbr \frac{1}{n}, 2\rpa \supseteq (0,2)$ \\\vp
            Hence, $\bigcup_{n=1}^\infty \lbr \frac{1}{n}, 2\rpa = (0,2)$ 
...
\section*{HW5: First attempt due \textbf{Friday 2/21}}
        \subsection*{Problem 8 (M)} \FGR(both parts)\\
        \SGR (both)\\
        \FGM\\
        \begin{enumerate}[label=(\alph*)]
...
\section*{HW9: First attempt due \textbf{Wednesday 3/05}}
    ...
    ({\it Hint: Use the result you proved in Problem 8(b), along with the definition of sequence convergence.})
        ...
        We want to show that $L_1=L_2$. //Make this more specific. We're going to prove that $|L_1 - L_2| < \Epsilon$.//\vp
        ...
        \begin{proof}
            ...
            Suppose that $\lim_{n\to\infty} a_n=L_1$ and $\lim_{n\to\infty} a_n=L_2$.\vp
            Let $\Epsilon> 0$.\\
            We want to show that $|L_1 - L_2| < \Epsilon$. \vp
            Via the definition of limit, $\exists N_1,N_2\in \N$ such that $\forall n_1>N_1, n_2>N_2$,
            \begin{align*}
...
\section*{HW11: First attempt due \textbf{Monday 3/10}}
    ...
    \subsubsection*{Second Attempt}
    \begin{proof}
    We will prove this statement both ways.\vp
        \underline{$\displaystyle \lim_{n\to\infty} -a_n = -\infty$ if $\displaystyle \lim_{n\to \infty} a_n = \infty$}\vp
        Let $m<0$.\vp
        We want to show that $\exists N\in \N$ such that $\forall n> N$, $-a_n < m$.\vp
...
\begin{proof} Let $\Epsilon > 0$.\vp
        ...
        Because $a_n$ and $b_n$ are Cauchy, let //``let'' is not appropriate here. Rather, $\exists N_a, N_b \in \N$ such that $\forall$ etc.// $N_a, N_b\in \N$,  $n_a, m_a > N_a$ and $n_b, m_b > N_b$ such that\vp
        \begin{align*}
...
\subsubsection*{Second Attempt}
    \begin{proof} Let $\Epsilon > 0$.\vp
        We want to show that $\exists N\in \N$ for $n,m > N$ such that $|(a_n+b_n) - (a_m + b_m)| < \Ep$.\vp
        ...
        \begin{align*}
            |(a_n+b_n) - (a_m + b_m)| \le |a_{n}-a_{m}| + |b_{n} - b_{m}| < \Ep.
        \end{align*}
        Hence, $a_n + b_n$ is Cauchy if both $a_n$ and $b_n$ are Cauchy.
    \end{proof}
...
Let $f(x) = x^2 +x -2$. Prove by definition that $\lim_{x\to 1} f(x) = 0$. ({\it Hint: Factor that quadratic, then use an idea similar to the $x^2$ limit from the worksheet.})
	\begin{proof} Let $\Ep>0.$ \blue{Let $\delta = \min{\left(\frac{\Ep}{4},1\right)}$.} //The WTS can (and should) include establishing the existence of $\delta$. So don't choose $\delta$ until after the WTS.//\vp
...
\subsubsection*{Second Attempt}
    \begin{proof} Let $\Ep>0.$\vp
            We want to show that \blue{for $ \delta = \min \left(1,\frac{\Ep}4\right ),$} //Sorry, I realize my wording was unclear. I meant, the WTS should say ``WTS $\exists \delta>0$ such that etc.'' So the WTS tells the reader that finding $\delta$ is the goal. We then find the $\delta$ in the next line.//
            
            $ \;0<|x-1|<\delta,$ implies that
            \begin{align*}
                \left|f(x)\right| < \Ep.
            \end{align*}
            Let $\delta = \min{\left(\frac{\Ep}{4},1\right)}$. //Don't forget to let $x$ !// 
...
\begin{proof} 
    Let $\Ep>0.$\vp
    We want to show that $\exists\delta>0$ such that $\;0<|x-1|<\delta,$ implies that
    \begin{align*}
        \left|f(x)\right| < \Ep.
    \end{align*}
    Let $\delta = \min{\left(\frac{\Ep}{4},1\right)}$. Let $0<|x-1|<\delta$. 
    Since $\delta \le 1$, we have
    \begin{align*}
        |x+2|<4.
    \end{align*}
...
\begin{proof} Let $\Ep>0$. //Add a WTS.//\vp
...
//Are those inequalities true for every $x\in D$, or are they subject to some kind of condition?//
    ...
    \begin{align*}
        ...
        &=|f(x)-L_1|+|f(x)-L_2|<\Ep.
    \end{align*}
    Hence, via problem 8(b), because $|L_2-L_1|<\Ep,\;L_1=L_2$.
...
\begin{proof} Let $\Ep>0$.\vp
    ...
    \begin{align*}
        ...
        |f(x_2)-L_2|&<\frac{\Ep}2.
    \end{align*}
    Let $\delta = \min\left(\delta_1, \delta_2\right)$. Let $0<|x-a|<\delta$. Therefore, both inequalities both hold for $x$. \\
    Thus, we have via the \textbf{Triangle Inequality} that
    \begin{align*}
        |L_2-L_1|=|f(x)-L_1-f(x)+L_2|&\le|f(x)-L_1|+|-f(x)+L_2|\\
        &=|f(x)-L_1|+|f(x)-L_2|<\Ep.
    \end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 28 %%%%%%%%%%
\subsection*{Problem 28 (M)} \FGR (part a)\\
\FGM\\
\begin{enumerate}[label=(\alph*)]
%%a
...
\item Prove that $\lim_{x\to 0} x\sin\lpa \frac{1}{x}\rpa = 0$.
    \begin{proof}
        Let $g(x) = -|x|$. Let $h(x) = |x|$.\\
        We want to show via the \textbf{Squeeze Theorem} that $f(x)$ converges to $0$.\vp
        Note that $g(x)=-|x|,$ so $\textstyle\lim_{x\to 0}g(x)=0$,\\ 
        And that  $f(x)=|x|,$ so $\textstyle\lim_{x\to 0}f(x)=0.$\vp
...
\begin{proof} \textit{Direct}\vp
    ...
    \begin{adjustwidth}{0.5cm}{0cm}
        \underline{$P \implies Q$}\vp
        Let $\Ep > 0$.\\
        We want to show that: //The following is incomplete as a WTS. You want to show that $\exists \delta>0$ such that etc. The existence of $\delta$ is part of the WTS.

        Also, for this direction, we don't know off the bat that the $\delta$ can be the same for both one-sided limits. So you WTS should have two potentially different $\delta$s in it.//
        \begin{enumerate}
    ...
    //The reverse direction is done! The forwards direction needs a couple edits to the WTS.//
\end{proof}
...
\begin{proof} \textit{Direct}\vp
    ...
    \begin{adjustwidth}{0.5cm}{0cm}
        \underline{$P \implies Q$}\vp
        Let $\Ep > 0$.\\
        We want to show that $\exists \delta_1, \delta_2>0$ such that:
        \begin{enumerate}
            \item For all $x$ satisfying $0<x-a<\delta_1$, $|f(x)-f(a)|<\Ep$, and
            \item For all $x$ satisfying $0<a-x<\delta_2$, $|f(x)-f(a)|<\Ep$.\\
        \end{enumerate}
        Because $\dlimx{a}f(x)=L$, we know that $\exists \delta > 0$ such that for $|x-a|<\delta$, we have:
        \setcounter{equation}{0}
        \begin{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 31 %%%%%%%%%%	
\subsection*{Problem 31 (M)} \FGR (part a)\\
\SGR (a)\\
\FGM\\
...
\begin{proof}
    ...
    \end{align*}
    //A new and interesting subtle problem has arisen. By assumption, $\lim_{x\to a} g(x)$ does not exist. So the limit law cannot be used to distribute across $\lim_{x\to a}(f(x)+g(x))$.

    Instead, write the $g$ limit as a combination of the $f$ limit and the $f+g$ limit to arrive at a contradiction.//
...
\begin{proof}
    \textit{Contradiction}.\vp
    Suppose for the sake of contradiction that $\dlimx{a}(f(x)+g(x))=L_0$.\\
    Then we have, via limit laws, that,
    \begin{align*}
        \dlimx{a}(f(x)+ g(x))-\dlimx af(x)&=L_0-L\\
        \dlimx a (f(x)+g(x)-f(x))&=L_0-L\\
        \dlimx a (g(x))&=L_0-L
    \end{align*}
    However, this contradicts that $\dlimx ag(x)$ does not exist.\vp
    Hence, $\dlimx{a}f(x)+g(x)$ does not exist. 
...
\subsubsection*{Second Attempt}
\begin{proof}
    We want to show that $\exists \alpha>0$ such that $\forall x\in Q\cap D,\;|g(x)|>\alpha$. //Apologies for missing this on the first attempt. $Q$ is undefined. Establishing the existence of $Q$ should be part of your WTS.//\vp
    Because $g$ is continuous at $x_0$, we know that $\exists \delta > 0$ such that for $|x-x_0|<\delta$,
    \begin{align*}
        |g(x_0)-g(x)|<\left|\frac{g(x_0)}2\right|.
    \end{align*}
   //Right here, define your $Q$ as $(x_0 - \delta, x_0+\delta)$.//
   ...
   Let $x\in D$ satisfy $|x-x_0|<\delta$. Therefore, we have that
    ...
    //Almost done! Probably best to run the third attempt by me before it's due.//
\end{proof}
\pagebreak
\subsubsection*{Third Attempt}
\begin{proof}
    We want to show that $\exists Q$, a neighborhood of $x_0$, such that $\exists \alpha>0$ such that $\forall x\in Q\cap D,\;|g(x)|>\alpha$. \vp
    Because $g$ is continuous at $x_0$, we know that $\exists \delta > 0$ such that for $|x-x_0|<\delta$,
    \begin{align*}
        |g(x_0)-g(x)|<\left|\frac{g(x_0)}2\right|.
    \end{align*}
    Let $Q=(x_0-\delta,x_0+\delta)$, and let $x\in Q\cap D$.\\
    Therefore, we have that
    \begin{align*}
        \left|\frac{g(x_0)}{2}\right|<|g(x)|<\left|\frac{3g(x_0)}2\right|. 
    \end{align*} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 33 %%%%%%%%%%
\subsection*{Problem 33 (M)} \FGR (part b)\\
\FGM\\
...
\begin{enumerate}[label=(\alph*)]
    ...
    So, for example, for $f(2)=4$, in any neighborhood around $2$, we can find an irrational $x$, and therefore $|f(2)-f(x)|=4$, which does not hold true for all $\Ep > 0$. //Correct!//
    
    \item Prove by definition that $f$ \textbf{is} continuous at $x_0=0$.
    \begin{proof}
        Let $\Ep > 0$. \vp
        We want to show that $\exists\delta > 0$ such that for $|x-0|<\delta$, $|f(0)-f(x)|<\Ep$. \vp
        Let $\delta = \sqrt{\Ep}$. \blue{Therefore, $|x|<\sqrt{\Ep}$.} //You need to actually let $x$.// \vp
        Thus, because $f(x)$ is either 0 or $x^2$, we have
        \begin{align*}
            |f(0)-f(x)|=|f(x)|\le |x^2|<\Ep.
        \end{align*}
...
\begin{proof}
    Let $\Ep > 0$. \vp
    We want to show that $\exists\delta > 0$ such that for $|x-0|<\delta$, $|f(0)-f(x)|<\Ep$. \vp
    Let $\delta = \sqrt{\Ep}$, and let $x\in \R$ satisfy $|x|<\delta$. \vp
    Therefore, $|x|<\sqrt{\Ep}$.\vp
    Thus, because $f(x)$ is either 0 or $x^2$, we have
    \begin{align*}
        |f(0)-f(x)|=|f(x)|\le |x^2|<\Ep.
    \end{align*}
...
\begin{proof}\textit{Direct @ $x = 1$}\vp
    //You forgot to let $\Epsilon$.// We want to show that $\exists\delta > 0$ such that for all $x$ satisfying $|x-1|<\delta$, we have:
    \begin{align*}
        |f(x)-2|<\Epsilon
    \end{align*}
    Let $\delta = \text{min}\left(\frac{\Ep}2, \sqrt \Ep,\left|\sqrt \Ep -1\right|\right)$. //And don't forget to let $x$ either!//\vp
    If $x$ is rational, 
    \begin{align*}
    ...
    \end{align*}
    //Something's off here. I definitely disagree with $|x-1|<\delta$ implying that $|x+1-1|<\delta$.

    Instead of using this $|\sqrt{\Epsilon}-1|$ quantity, why not just use $1$? Like we've done with other limits of quadratics.//
    ...
    //A couple missing ``let''s, and an error.//
\end{proof}

\subsubsection*{Third Attempt}
\begin{proof}\textit{Direct @ $x = 1$}\vp
    Let $\Ep>0$. We want to show that $\exists\delta > 0$ such that for all $x$ satisfying $|x-1|<\delta$, we have:
    \begin{align*}
        |f(x)-2|<\Epsilon
    \end{align*}
    Let $\delta = \text{min}\left(\frac{\Ep}3,1\right)$. Let $x\in \R$ satisfy $|x-1|<\delta.$\vp
    If $x$ is rational, 
    \begin{align*}
        |f(x)-2|=2|x-1|<2\left|\frac{\Ep}3\right| < \Ep
    \end{align*}
    If $x$ is irrational:
    \begin{align*}
        |f(x)-2|&=|x^2+1-2|\\
        &=|x^2-1|\\
        &=|(x+1)(x-1)|<\left|(x+1)\frac{\Ep}3\right|
    \end{align*}
    Because:
    \begin{align*}
        |x-1|&<1\\
    ...
    So, we have that:
    \begin{align*}
        |f(x)-2|&<\left|(x+1)\Ep\right|\\
        &<\left|(3)\frac{ \Ep}3\right|\\
        &=|\Epsilon|
    \end{align*}
    Hence, because the $\Ep$-criterion is fulfilled regardless of whether $x$ is rational or irrational,\\
    $\dlimx 1 f(x) = 2$. 
...
\begin{proof}\textit{Direct.}\vp
    ...
    \end{adjustwidth}
    Next, because $f(b)$ is an upper bound, we want to show that \blue{$\sup E = f(b)$} //Huge problem. This need not be true. $f$ could have a jump discontinuity at $b$.
    
    Since $E$ is bounded above, $E$ has a $\sup$ by the Completeness Axiom. You can then show, using the definition of limit, that $\lim_{x\to b} f(x) = \sup E$.//
    
    by showing that $f(b)$ is the least upper bound.\vp
    \begin{adjustwidth}{0.5cm}{0cm}
    ...
    \begin{adjustwidth}{0.5cm}{0cm}
        Let $x_0<b$ such that $f(x_0) < f(b)-\frac{\Ep}2$. //How do you know such an $x_0$ exists?// \\
        Therefore,
        \begin{align*}
    ...
    //The bounded above part is good, and the right idea for the limit part is in there. To make it more precise, since we know $\sup E$ exists, we get to use the $\Epsilon$-criterion for the $\sup$!//

\end{proof}

\subsubsection*{Third Attempt}
\begin{proof}\textit{Direct.}\vp
    We want to show that $\dlimx{b^-}f(x)=\sup E$.\vp
    \textit{First}, we want to show that $E$ is bounded above to show that a supremum exists.
    \begin{adjustwidth}{0.5cm}{0.5cm}
        Consider that because $f$ is a strictly increasing function in $[a,b]$, that:
        \begin{align*}
            \forall x\in (a,b),\; f(x)<f(b)
        \end{align*}
        Thus, $E$ is bounded above and its supremum exists.\\
    \end{adjustwidth}
    \textit{Next}, we want to show that $\dlimx{b^-}f(x)=\sup E$.
    \begin{adjustwidth}{0.5cm}{0.5cm}
        Let $\Ep>0$. Via the $\Ep$-criterion of supremum, $\exists c\in (a,b)$ such that:
        \begin{align*}
            \sup E - \Ep<f(c). 
        \end{align*}
        Let $\delta = b - c$.\\
        Thus, we have that $\forall x\in [a,b]$ satisfying $0<b-x<\delta$, we have:
        \begin{align*}
            \sup E - \Ep&<f(x)\\
            \sup E - f(x)&<\Epsilon
        \end{align*}
        And because $f(x)\le \sup E$, we have that:
        \begin{align*}
            |f(x)-\sup E|<\Ep
        \end{align*}
        Thus, $\dlimx{b^-}f(x)=\sup E$.
    \end{adjustwidth}
...
\begin{proof}Let $\Ep > 0$.\vp
    ...
    \end{align*}
    Let $\delta = a^2\cdot\Ep$. //Don't forget to let $x$ and $y$!// Thus, we have:
    \begin{align*}
...
\begin{proof}Let $\Ep > 0$.\vp
    ...
    \begin{align*}
        \left|\frac{1}x - \frac{1}y\right|<\Ep.
    \end{align*}
    Let $\delta = a^2\cdot\Ep$. Let //``$x,y\in [a,\infty)$ such that''// $|x-y|<\delta$. Thus, we have:
    \begin{align*}
        \left|\frac{1}x-\frac{1}y\right|&=\left|\frac{x-y}{xy}\right|\\
        &<\left|\frac{a^2\cdot \Ep}{xy}\right|.
    \end{align*}
...
\begin{proof}\textit{Direct}.\vp
    ...
    \end{align*}
    Let $\delta > 0$. \blue{Therefore, we have that:}
    \begin{align*}
        \blue{\left|\frac{1}x-\frac{1}y\right|} &=\blue{\left|\frac{x-y}{xy}\right|}
    \end{align*}
    //Cut this. Without introducing $x$ and $y$, that algebra is meaningless. And you can include $\left|\frac{1}x-\frac{1}y\right|=$ at the start of the algebra below.//
    
    \blue{Then, let $x = y+\frac{\delta}2$. Let $y=\min\left(\frac{\delta}2, \frac{1}2\right)$.} //Define $y$ first since $x$ depends on $y$. Also, $x$ needs a second case for when $y=1/2$.//\\
    Thus, we have:
    \begin{align*}
        \blue{\left|\frac{x-y}{xy}\right|} &= \blue{\left|\frac{\frac{\delta}2}{y(y+\frac{\delta}2)}\right|}\\
        &\ge\left|\frac{\frac{\delta}2}{\frac{1}2\cdot(\frac{\delta}2+\frac{\delta}2)}\right|=\frac{1}4
    \end{align*}
    //That first line either needs to be an inequality, or you need to do cases. Because $y$ is not always $\delta/2$.//
    
    Thus, we have that $\left|\frac{1}x-\frac{1}y\right|\ge\Ep$.\vp
    Hence, $f$ is not uniformly continuous on $(0,1)$.

    //You have the right idea! It's really just proof-grammar stuff that needs editing.//
\end{proof}
...
\begin{proof}\textit{Direct}.\vp
    ...
    \end{align*}
    Let $\delta > 0$.\\
    Then, let $y=\f{\delta}{\delta+1}$, and let $x=\frac{y}2.$ We have that:
    \begin{align*}
        |x-y|=\left|\f{1}2\pr{\f{\delta}{\delta+1}}\right|<\left|\f{1}2\,(\delta)\right|<\delta.
    \end{align*}
    Thus, we have, because $\delta > 0$, that:
    \begin{align*}
        \left|\frac{1}x-\frac{1}y\right|&\ge \left|\,2\pr{\f{\delta+1}\delta}-\f{\delta+1}\delta\right|=\left|\f{\delta+1}{\delta}\right| > 1=\Ep.
    \end{align*}
    Hence, $f$ is not uniformly continuous on $(0,1)$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW21: First attempt due \textbf{Monday 4/21}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 40 %%%%%%%%%%
\subsection*{Problem 40} \FGR (both)\\
\SGR (part b)\\
\FGM\\
Define $f: \R \to \R$ by 
	\[f(x) = \begin{cases} x^2 & \text{ if } x\geq 0\\ 0 &\text{ if } x<0 \end{cases}. \]
\begin{enumerate}[label=(\alph*)]
	\item Prove that $f$ is differentiable on $\R$. ({\it Hint: Do this in two cases. If $x\neq 0$, you can use regular derivative rules. If $x = 0$, you need to use the limit definition of the derivative.})
    \begin{proof}
        Let $a\in \R$. \\
        We want to show that $\dlimx{a}\frac{f(x)-f(a)}{x-a}$ exists.\vp
        Consider for \blue{$a\ge 0$} //Problem. At $a=0$, since $\lim_{x\to 0}$ is two-sided, and $f$ has a different formula on either side of $0$, this case must be done using one-sided limits.//
        that:
        \begin{align*}
            \dlimx{a}\frac{f(x)-f(a)}{x-a}&=\dlimx{a}\frac{x^2-a^2}{x-a} \\
            &=\dlimx{a}\frac{(x+a)(x-a)}{x-a}\\
            &=\dlimx{a}x+a\\
            f'(a)&=2a
        \end{align*}
        //You could have just used the power rule for $a>0$.//
        
        Therefore we have that the limit for $a\ge 0$ exists and is finite. 
        Now, consider for $a\le 0$
        \begin{align*}
            \dlimx{a}\frac{f(x)-f(a)}{x-a}&=\dlimx{a}\frac{0}{x-a} \\
            &=0\\
            f'(a)&=0
        \end{align*}
        //You can use the fact that the derivative of a constant is zero.//
        
        Therefore we have that for $a\le0$, the limit exists and is finite. \vp
        Finally, for $a=0$, we have that the limit from the left $=0$.\\
        From the right, we have $f'(a)=2a=0$. \\
        Thus, $f'(0)$ also exists. \vp
        Hence, $\forall a\in \R$, $f'(a)$ exists and is finite, so $f$ is differentiable on $\R$. 

    \end{proof}
	
	\item Write down the piecewise-defined function $f'(x)$. Is $f'$ differentiable on $\R$? Prove your claim. ({\it Hint: It may help to look at the graph of $f'$.})
		\begin{proof}
		Consider that for $f'(a)$, we have:
        \begin{align*}
            f'(a)=\casi{2a}{a\ge 0}{0}{a\le 0}.
        \end{align*}
        Using standard derivate //typo// laws, we have that:
        \begin{align*}
            f''(a)=\casi{2}{\blue{a\ge0}}{0}{a<0}.
        \end{align*}
        //At $a=0$, the two one-sided derivative limits disagree. So $f''(0)$ actually turns out to be undefined. You need to demonstrate that.//
        
        However, this function is discontinuous at $a=0$, because:
        \begin{align*}
            \dlimx{0^-}f''(x)&=0, \text{ but}\\
            \dlimx{0^+}f''(x)&=2.
        \end{align*}
        So we have that $\dlimx{0}f''(x)$ does not exist.\\
        Because $f''(a)$ is not continuous at a singular point, $f'(a)$ is not differentiable on $\R$. 
    \end{proof}
\end{enumerate}

\subsubsection*{Second Attempt (M) (part a)}
\begin{proof}
    Let $a\in \R$. \\
    We want to show that $\dlimx{a}\frac{f(x)-f(a)}{x-a}$ exists.\vp
    Consider that via derivative laws, we have:
    \begin{align*}
        f'(x)=\casi{2x}{x> 0}{0}{x<0}
    \end{align*}
    Therefore we have that the limit for $x\ne 0$ is defined.\vp
    Now, consider that:
    \begin{align*}
        \dlimx{0^+}\frac{f(x)-f(0)}{x-0}=\dlimx{0^+}\f{x^2}x&=0\\
        \dlimx{0^-}\f{f(x)-f(0)}{x-0}=\dlimx{0^-}\f{0}x&=0
    \end{align*}
    Thus, because the derivative is equal both from the left and right, $f'(0)$ also exists. \vp
    Hence, $\forall x\in \R$, $f'(x)$ exists and is finite, so $f$ is differentiable on $\R$.  //Correct!//
\end{proof}
\subsubsection*{Second Attempt (part b)}
\begin{proof}
    Consider that for $f'(x)$, we have:
    \begin{align*}
        f'(x)=\casi{2x}{x\ge 0}{0}{x< 0}.
    \end{align*}
    However, \blue{this function is discontinuous at $a=0$,} //That's not accurate. $f'$ itself is continuous at $a=0$, but its derivative isn't because it has a hole at $a=0$.// because:
    \begin{align*}
        \dlimx{0^+}\f{f(x)-f(0)}{x-0}&=\dlimx{0^+}\f{2x}{x}=2, \text{ but}\\
        \dlimx{0^-}\f{f(x)-f(0)}{x-0}&=\dlimx{0^-}\f{0}{x}=0,
    \end{align*}
    Thus, because $\dlimx{0}\frac{f(x)-f(0)}{x-0}$ does not exist, $f'(a)$ is not differentiable on $\R$.

    //A wording change is needed in one spot.//
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
        f'(x)=\casi{2x}{x\ge 0}{0}{x< 0}.
    \end{align*}
    However, this function's derivative is discontinuous at $a=0$, because:
    \begin{align*}
        \dlimx{0^+}\f{f(x)-f(0)}{x-0}&=\dlimx{0^+}\f{2x}{x}=2, \text{ but}\\
        \dlimx{0^-}\f{f(x)-f(0)}{x-0}&=\dlimx{0^-}\f{0}{x}=0,
    \end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 41 %%%%%%%%%%
\subsection*{Problem 41} \FGR (both)\\
\SGR (b)\\
\FGM\\
When necessary for this problem, you may use the following facts. We've proved some of these in either class or homework, but I trust you would be capable of proving them yourself by now. Feel free to do so for extra practice if you want!
\begin{itemize}
	\item $\lim_{x\to 0} \sin(1/x)$ does not exist;
	\item $\lim_{x\to 0} \cos(1/x)$ does not exist;
	\item $\lim_{x\to 0} x \sin(1/x) = 0$;
	\item $\lim_{x\to 0} x^2 \sin(1/x) = 0$.
\end{itemize}
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{proof} Let $x_0=0$.\vp
        We want to show that:
        \begin{align*}
            \dlimx{x_0}\frac{f(x)-0}{x-0}=0
        \end{align*}
        Consider that $f(x_0)=0$. Then, we have that:
        \begin{align*}
            f'(x_0)=\dlimx{x_0}\frac{f(x)-f(x_0)}{x-x_0}&=\dlimx{x_0}\frac{f(x)-0}{x-0}\\
            &=\dlimx{x_0}\frac{x^2\sin(\frac{1}x)}{x}\\
            &=\dlimx{x_0}x\sin\left(\frac{1}x\right)\\
            &=\blue{\dlimx{x_0}0\cdot \sin\left(\frac{1}x\right)}\\
            &=\blue{0\cdot\dlimx{x_0} \sin\left(\frac{1}x\right)}\\
            &=0
        \end{align*}
        //You seem to be using limit laws when one of the functions has no limit. That's not correct.

        Once you have it down to $\dlimx{x_0}x\sin\left(\frac{1}x\right)$, just use one of the facts I gave you.//
        
        Hence, $f'(x_0)=0$. 
    \end{proof}
	
	\item For $x\neq 0$, find a formula for $f'(x)$. Does $\lim_{x\to 0} f'(x)$ exist? Prove your claim.
		\begin{proof}\textit{Direct.}\vp
            Consider the derivate //typo// laws. Therefore, we have for $f'(x)$:
            \begin{align*}
                f'(x)=\casi{2x\sin\left(\frac{1}x\right)-\cos\left(\frac{1}x\right)}{x\ne 0}{0}{x=0}
            \end{align*} 
            
            If $\dlimx{0}f'(x)$ exists, then both $\dlimx{0^+}f'(x)$ and $\dlimx{0^-}f'(x)$ exist and are equal.\vp
            Consider that:
            \begin{align*}
                \dlimx{0^-}f'(x)&=\dlimx{0^-}\pr{2x\sin\pr{\frac{1}x}-\cos\pr{\frac{1}x}}\\
                &=\blue{\dlimx{0^-}\pr{2x\sin\pr{\frac{1}x}}-\dlimx{0^-}\pr{\cos\pr{\frac{1}x}}}
            \end{align*}
            //Again, you're applying the limit laws even though one of the limits does not exist. Instead, cite Problem 31 (a) here.//
            
            However, $\dlimx{0^-}\pr{\cos\pr{\frac{1}x}}$ does not exist, and thus $\dlimx{0^-}f'(x)$.\\
            Hence, $\dlimx{0}f'(x)$ does not exist. 
		\end{proof}
\end{enumerate}
...
\begin{proof} Let $x_0=0$.\vp
    ...
    \begin{align*}
        ...
        &=0
    \end{align*}    
    Hence, $f'(x_0)=0$. //Correct!//
\end{proof}
\newpage
\subsubsection*{Second Attempt (part b)}
\begin{proof}\textit{Direct.}\vp
    Consider the derivative laws. Therefore, we have for $f'(x)$:
    \begin{align*}
        f'(x)=\casi{2x\sin\left(\frac{1}x\right)-\cos\left(\frac{1}x\right)}{x\ne 0}{0}{x=0}
    \end{align*} 
    ...
    \begin{align*}
        \dlimx{0^-}f'(x)&=\dlimx{0^-}\pr{2x\sin\pr{\frac{1}x}-\cos\pr{\frac{1}x}}
    \end{align*}
    Thus, via problem 31, because $\dlimx{0^{-}}\cos\pr{\frac{1}x}$ does not exist, $\dlimx{0}f'(x)$ does not exist. //In order to use problem 31 (a), you also have to confirm that one of the two limits does exist. You can do so using the facts I gave you.//
\end{proof}
...
\begin{proof}\textit{Direct.}\vp
    ...
    \begin{align*}
        \dlimx{0^-}f'(x)&=\dlimx{0^-}\pr{2x\sin\pr{\frac{1}x}-\cos\pr{\frac{1}x}}\\
        &=\dlimx{0^-}\pr{2x\sin\pr{\frac{1}x}}-\dlimx{0^-}\pr{{\cos\pr{\frac{1}x}}}\\
    \end{align*}
    Then, consider that:\vp
    $\tlimx{0}2x\sin(1/x)=2\tlimx{0}x\sin(1/x)=0$, and that:\\
    $\tlimx{0}(-\cos(1/x))=-\tlimx{0}(\cos(1/x))$\vp
    Thus, via problem 31, the sum:
    \begin{align*}
        \dlimx{0^-}\pr{2x\sin\pr{\frac{1}x}-\cos\pr{\frac{1}x}}
    \end{align*}
    does not exist because the sum of a limit that does exist and one that doesn't does not exist.
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW22: First attempt due \textbf{Wednesday 4/23}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 42 %%%%%%%%%%
\subsection*{Problem 42 (M)} \FGM\\
Prove that the function $f(x) = 2x^3 +3x^2 -36x +17$ is one-to-one on the interval $[-1,1]$. Is $f$ increasing or decreasing?
	\begin{proof}\textit{Direct}.\vp
        Let $x\in [-1,1]$. We want to show that $f'(x)\ne0$, and that $f'(x)<0$.\vp
        Consider that:
        \begin{align*}
            f'(x)&=6x^2+6x-36\\
            &=6(x^2+x-6)\\
            &=6(x+3)(x-2)
        \end{align*}
        Note that no roots of $f'(x)$ exist in the interval $[-1,1]$.\\
        Thus, because $f'(x)\ne 0$, $f$ is one-to-one. \vp
        Consider that $f'(0)=-36$. \\
        Because $f'$ is continuous, $f' \ne 0$, and there exists a negative value on the interval $[-1,1]$, $f' < 0$. \vp
        Thus, $f$ is strictly decreasing. 
	\end{proof}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 43 %%%%%%%%%%	
\subsection*{Problem 43 (M)} \FGM\\
Prove that $f(x) = x^3 +x -1$ has exactly one real root. ({\it Hint: Show that $f$ has at least one root using one theorem, then show that $f$ has at most one root using a different theorem.})
	\begin{proof} We want to show that $f$ has exactly one real root.\vp
        Via the \textbf{Intermediate Value Theorem}, because $f(-1) < 0$, and $f(1)>0$, $\exists c\in \R$ such that $f(c)= 0$. \vp
        Consider that:
        \begin{align*}
            f'(x)=3x^2+1.
        \end{align*}
        Because $f'(x)$ is always positive, $f$ is strictly increasing, $f$ is one-to-one, and therefore there is only one real root. 
	\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW23: First attempt due \textbf{Friday 4/25}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 44 %%%%%%%%%%
\subsection*{Problem 44} \FGR\\
\FGM\\
Calculate $\lim_{x\to 0} \frac{x^2\sin x}{\sin x -x\cos x}.$ ({\it Hint: Keep going until it works.})\\
\begin{proof}\newcommand{\eq}{\frac{x^2\sin x}{\sin x -x\cos x}}\newcommand{\ddx}{\frac{d}{dx}}
    We want to show via. L'Hopital's //To make the o with the accent, \^{o}.// Rule that $\dlimx{0}\eq=???$. //Probably best to come fill this in once you know what the limit is.//\vp
    First, consider that:
    \begin{align*}
        \dlimx{0}\eq = \frac{\dlimx{0}x^2\sin x}{\dlimx{0}(\sin x-x\cos x)}=\frac{0}{0}
    \end{align*}
    Using L'Hôpital's Rule, we get that:
    \begin{align*}
        \dlimx{0}\f{\ddx x^2\sin x}{\ddx \pr{\sin x - x \cos x}}=\dlimx{0}\f{2x\sin x + x^2\cos(x)}{x\sin x}.
    \end{align*}
    However, taking this limit now results in:
    \begin{align*}
        \frac{\dlimx{0}(2x\sin x + x^2\cos x)}{\dlimx{0}x\sin x}= \frac{0}0
    \end{align*}
    Using L'Hôpital's Rule again, we get yet again:
    \begin{align*}
        \dlimx{0}\f{\ddx 2x\sin x + x^2\cos x}{\ddx x \sin x}=\dlimx{0}\f{2\sin x + 4x\cos x - x^2\sin x}{\sin x + x\cos x} =  \f{0}{0}
    \end{align*}
    Finally, using L'Hôpital's Rule one last time, we get that:
    \begin{align*}
        \dlimx{0}\f{\ddx \pr{2\sin x + 4x \cos x - x^2 \sin x}}{\ddx \pr{\sin x + x \cos x}}=\dlimx{0}\f{6\cos x -6x\sin x -x^2\cos x}{2\cos x + x\sin x}=\frac{6}2=3
    \end{align*}
    Hence, $\dlimx{0}\eq = 3$. 

    //Just a small thing in line 1.//
\end{proof}

\subsubsection*{Second Attempt}
\begin{proof}\newcommand{\eq}{\frac{x^2\sin x}{\sin x -x\cos x}}\newcommand{\ddx}{\frac{d}{dx}}
    We want to show via. L'Hôpital's Rule that $\dlimx{0}\eq=3$.\vp
    First, consider that:
    \begin{align*}
        \dlimx{0}\eq = \frac{\dlimx{0}x^2\sin x}{\dlimx{0}(\sin x-x\cos x)}=\frac{0}{0}
    \end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 45 %%%%%%%%%%	
\subsection*{Problem 45} \FGR\\
\FGM\\
Suppose both $f$ and $f^{-1}$ are twice-differentiable functions. Derive a formula for $(f^{-1})''$.\\

\begin{proof}
    We want to utilize the \textbf{Inverse Function Theorem} twice to find $(f^{-1})''$.\vp
    Because $f$ is twice differentiable, we know via the \textbf{IFT} that $f$ and $f'$ are one-to-one, and that we have:
    \begin{align*}
        (f\inv)'(y)=\f{1}{f'(f\inv(y))}.
    \end{align*}
    Now, with the \textbf{IVT}, we apply the same formulation for $(f\inv)''(y)$:
    \begin{align*}
        \pr{f\inv}''(y)=\frac{1}{f''((f\inv)'(y))}=\f{1}{f''\pr{\f{1}{f'(f\inv(y))}}}.
    \end{align*}

    //This is not correct. You need to use the IFT \textbf{and} the Chain Rule.//
\end{proof}

\subsubsection*{Second Attempt}
\begin{proof}\definecolor{periwinkle}{rgb}{0.58, 0.58, 0.86}
    We want to utilize the \textbf{Inverse Function Theorem} twice to find $(f^{-1})''$.\vp
    Because $f$ is twice differentiable, we know via the \textbf{IFT} that $f$ and $f'$ are one-to-one, and that we have:
    \begin{align*}
        (f\inv)'(y)=\f{1}{f'(f\inv(y))}.
    \end{align*}
    Now, we apply the chain rule to find $(f\inv )'(y)$:
    \begin{align*}
        \f{d}{dy}(f\inv)'(y) &= \purp{\br{\f{d}{dy}}}\viol{\lbr\f{1}{f'(f\inv(y))}\rbr}\\
        &={-\f{1}{f'(f\inv(y))^2}}\cdot \purp{\lbr\f{d}{dy}\rbr}\viol{\br{\pr{f'(f\inv(y))}}}\\
        &=-\f{1}{f'(f\inv(y))^2}\cdot f''(f^{-1}(y))\cdot \purp{\br{\f{d}{dy}}}\viol{[f\inv(y)]}\\
        &=-\f{1}{f'(f\inv(y))^2}\cdot f''(f^{-1}(y))\cdot \f{1}{f'(f\inv(y))}\\
        (f\inv)''(y)&=-\f{f''(f^{-1}(y))}{f'(f\inv(y))^3}
    \end{align*}
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW24: First attempt due \textbf{Wednesday 4/30}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 46 %%%%%%%%%%
\subsection*{Problem 46} \FGR\\
Define $f:[0,3]\to \R$ by 	
	\[f(x) = \begin{cases} 1 & \text{ if } 0 \leq x \leq 1\\
		2 & \text{ if } 1 < x \leq 2\\
		3 & \text{ if } 2 < x \leq 3 \end{cases}. \]
Prove using the upper-and-lower-sum theorem that $f\in \mathcal{R}_{[0,3]}$ and compute the integral.
\begin{proof}
    Let $\Ep>0$.
    We want to show that:
    \begin{align*}
        \underline{\int_0^3}f(x)\;dx&=\overline{\int_0^3}f(x)\;dx, \;\;\text{such that for a partition }P:\\
        U(f,P)&-L(f,P)<\Ep
    \end{align*}
    \underline{\bld{Part 1: $f(x)\in\mathcal R_{[0,3]}$}}\vspace{0.2cm}
    \begin{adjustwidth}{0.5cm}{0.5cm}
        Let $a = \f{\Ep}{20}$, and let:
        \begin{align*}
            P=\left\{
                [0,1-a],[1-a,1],[1,1+a]
                [1+a, 2-a],[2-a,2],[2,2+a]
        ...
        Thus, 
        \begin{align*}
            U(f,P)-L(f,P)=10a=\f{\Ep}2<\Ep.
        \end{align*}
        Hence, $f(x)$ is \bld{Riemann Integrable} on $[0,3]$. \vp
    \end{adjustwidth}
    \underline{\bld{Part 2: $\tinti{0}{3}f(x)=6$}}\vspace{0.2cm}
    \begin{adjustwidth}{0.5cm}{0.5cm}
        Next, we want to show that \blue{$\sup\{L(f,P)\}=6$ for the $P$} . //``$\sup \{L(f,P : P \text{ is a partition of } [a,b]\} =6.$''// \vp
        First, consider that $\blue{\sup\{L(f,P)\}\le\inf\{U(f,P)\}}$. //Those sets need the same edit as above. And the same goes for the rest of the proof.//\\
        Because, via the process in \bld{Part 1}, we can get $U(f,P)$ closer to $6$ than $6+\Ep$, we have:
        \begin{align*}
            \blue{\sup\{L(f,P)\}}\le 6
        \end{align*}
        Thus, 6 is an upper bound of $\blue{\sup\{L(f,P)\}}$.\vp
        Furthermore, via the partition in \bld{Part 1} again, we have:
        \begin{align*}
            \blue{\sup\{L(f,P)\}}> 6-\Ep.
        \end{align*}
        Thus, because we can fulfill the $\Ep$-criterion, $\blue{\sup\{L(f,P)\}}=6$. \vp
        Hence, because $f(x)\in\mathcal R_{[a,b]}$, 
        \begin{align*}
            6 = \blue{\sup\{L(f,P)\}}=\int_0^3f(x)\;dx.
        \end{align*}

        //Correct! A notation change needs to be made throughout Part 2.//
    \end{adjustwidth}
\end{proof}
...
\begin{proof}
    ...
    \underline{\bld{Part 2: $\tinti{0}{3}f(x)=6$}}\vspace{0.2cm}
    \begin{adjustwidth}{0.5cm}{0.5cm}
        Next, we want to show that $\sup\{L(f,P:P\text{ is a partition of }[a,b])\}=6$.\vp
        First, consider that $\sup\{L(f,P):P\text{ is a partition of }[a,b])\}\le\inf\{U(f,P):P\text{ is a partition of }[a,b])\}$.\\
        Because, via the process in \bld{Part 1}, we can get $U(f,P):P\text{ is a partition of }[a,b])$ closer to $6$ than $6+\Ep$, we have:
        \begin{align*}
            \sup\{L(f,P):P\text{ is a partition of }[a,b])\}\le 6
        \end{align*}
        Thus, 6 is an upper bound of $\sup\{L(f,P):P\text{ is a partition of }[a,b])\}$.\vp
        Furthermore, via the partition in \bld{Part 1} again, we have:
        \begin{align*}
            \sup\{L(f,P):P\text{ is a partition of }[a,b])\}> 6-\Ep.
        \end{align*}
        Thus, because we can fulfill the $\Ep$-criterion, $\sup\{L(f,P):P\text{ is a partition of }[a,b])\}=6$. \vp
        Hence, because $f(x)\in\mathcal R_{[a,b]}$, 
        \begin{align*}
            6 = \sup\{L(f,P):P\text{ is a partition of }[a,b])\}=\int_0^3f(x)\;dx.
        \end{align*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW25: First attempt due \textbf{Friday 5/02}}	
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 47 %%%%%%%%%%
\newcommand{\case}[3]{\begin{cases}#1 & \text{ if }#2 \\ #3 & \text{ else }\end{cases}}
\subsection*{Problem 47} \FGR\\
Suppose $f$ is Riemann integrable on $[a,b]$, and that $g$ is a function such that $f(x) = g(x)$ for all but a single value of $x\in [a,b]$. Prove that $g$ is Riemann integrable on $[a,b]$, and that $\int_a^b g(x)\,dx = \int_a^b f(x)\,dx$. \\
({\it Hint: Give a name to the $x$-value where $f$ and $g$ disagree. Then define the function $h:[a,b]\to \R$ by $h(x) = f(x) - g(x)$.\\
I'll give more hints in feedback as needed.})
\begin{proof}Let $c,d\in \R$ such that $f(x_0)= c\ne d,\;x_0\in[a,b]$. //This is a bit of a backwards way of thinking about $f$ and $g$. It reads like you're deciding first what value $f$ and $g$ should have at $x_0$, and then letting $f$ be whatever it wants on the other $x$-values. Instead, if you want to re-introduce $f$ and $g$, restate the starting assumption from the claim. Then just give a name to the number where they are unequal.//
    \begin{align*}
        \text{Let }g(x)=\case{d}{x=x_0}{f(x)}{}
    \end{align*}
    Let $h:[a,b]\to\R$ be a function such that for $h(x)=f(x)-g(x),$ we have:
    \begin{align*}
        h(x)=\case{\blue{a-b}}{x=x_0}{0}
    \end{align*}
    //I disagree with $a-b$. With you notation, I think you mean $c-d$.//
    
    Let $k=\f{\Ep}{4(\blue{a-b})}$, //Again.// and let $P$ be a partition such that:
    \begin{align*}
        P=\{[a,x_0-k],[x_0-k,x_0],[x_0,x_0+k],[x_0+k,b]\}.
    \end{align*}
    //Three things. First, partitions are sets of individual points, not of subintervals. So your $P = \{a, x_0-k, x_0, x_0+k, b\}$.
    
    Second, I don't think you need $x_0$ to be part of the partition. It's not wrong to include it, it's just unnecessary.

    Third, this partition will work under the implicit assumption that $x_0$ does not equal either $a$ or $b$. But if it does, then this partition will not work. Probably easiest to handle that in a separate case.//
    
    Thus, for $h$ we have that:\vp
    \begin{center}
        \begin{tabular}{lcr}
            $\begin{aligned}
                L(h,P)&=0\cdot (x_0-k-a)\\
                &+ 0\cdot (k)\\
                &+(a-b)(-k)\\
                &+0\cdot(b-x_0-k)\\
                &=-\frac{\Ep}4
        ...
        \end{tabular}
    \end{center}
    //Okay, now the $a-b$ thing is actively wrong-footing you. One of these two sums should have been $0$, contingent on whether $h(x_0)$ is positive or negative.//
    
    \begin{align*}
        U(h,P)-L(h,P)=\f{\Ep}2<\Ep
    \end{align*}
    Hence, $h\in\mathcal R_{[a,b]}$.\vp
    \blue{Additionally, we have that $\inf{U(h,P)}=0$, becuase we can get via $k$, a $U(h,p)$ as small as we want it because it's tied to $\Ep$.} //Typo. Also, once you fix your upper and lower sums, this reasoning will be easier because one of those objects will always be $0$.//
    
    Thus,
    \begin{align*}
        0=\inf\{U(h,P)\}=\overline{\int_a^b} h(x)\;dx=\int_a^b h(x)\;dx\\
        \int_a^b h(x)\;dx=\int_a^bf(x)-g(x)\;dx.
    \end{align*}
    Now we have that:
    \begin{align*}
        U(f-g,P)-L(f-g,P)\\
        ...
        <\Ep\\
    \end{align*}
    This means that the difference between our \blue{Left \& Right sums} //What left and right sums? That's not what $U(f,P)$ and $L(f,P)$ are conceptually.//
    
    from $f$ and $g$ is arbitrarily small, so we also know that:
    \begin{align*}
        U(g,P)-L(g,P)-(U(f,P)-L(f,P))<\Ep
    \end{align*}
    However, note that $f$ is Riemann integrbale, so we know that $\forall\epsilon > 0$, 
    \begin{align*}
        U(f,P)-L(f,P)<\epsilon
    \end{align*}
    Thus, $U(f,P)-L(f,P)=0$, and then we have that:
    \begin{align*}
        U(g,P)-L(g,P) < \Epsilon.
    \end{align*}
    Thus, $g$ is Riemann integrable on $[a,b]$.\vp
    I dunno if i wanna do the rest of this because i def messed something up so i don't want to go evaluating the integrals. I assume we can't just go:
    \begin{align*}
        \int_a^b f(x)-g(x)\;dx = \int_a^b f(x)\;dx-\int_a^bg(x)\;dx=0\\
        \int_a^b f(x)=\int_a^bg(x)\;dx
    \end{align*}

    //For the part where you extend everything to $g$, just use the linearity result (the one you're proving in Problem 48 (a))//
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW26: First attempt due \textbf{Monday 5/05}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 48 %%%%%%%%%%	
\subsection*{Problem 48} \FGR\\
({\it See the Day 31 Worksheet for hints.})

Let $f,g:[a,b]\to \R$ be bounded functions, and let $c\in \R$. Suppose that $f,g\in \mathcal{R}_{[a,b]}$.
\begin{enumerate}[label=(\alph*)]
	\item Prove that $f+g \in \mathcal{R}_{[a,b]}$, and that
		\[\int_a^b (f+g)(x)\,dx = \int_a^b f(x)\,dx + \int_a^b g(x)\,dx. \]
	\begin{proof}
	nope
	\end{proof}
	
	\item Prove that $cf \in \mathcal{R}_{[a,b]}$, and that
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{HW27: First attempt due \textbf{Wednesday 5/07}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 49 %%%%%%%%%%
\subsection*{Problem 49} 
Prove by induction that $n! > 2^n$ for all $n\in \N$ such that $n>3$. ({\it Does this look familiar?})
	\begin{proof}Let $P_n$ denote the statement $n!>2^n$.\\
        We want to show via induction that $P_n$ is true for all $n\in \N \,\backslash \{1,2\}$. \vp
        First consider the base case $P_4$. We have:
        \begin{align*}
            (4)!&>2^4\\
            24 &>16
        \end{align*}
        Thus, $P_4$ is true.\vp
        Now, we want to show the inductive step, such that $n!>2^n \implies (n+1)!>2^{n+1}$.\\
        We have with $P_n$ that:
        \begin{align*}
            n! &> 2^n\\
            (n+1)\cdot n! &> (n+1)2^n.
        \end{align*}
        For $n>3$, we have that $(n+1) > 2$, so we also have that:
        \begin{align*}
            (n+1)2^n>2\cdot2^n = 2^{n+1}.
        \end{align*}
        Then, we have that:
        \begin{align*}
            (n+1)! = (n+1) \cdot n! > (n+1)2^n > 2^{n+1}.
        \end{align*}
        Thus, the inductive step holds.\vp
        Becuase we have $\vdash P_4$ and $P_n \Longrightarrow P_{n+1}$, $n!> 2^n$ for all $n\in \N\,\backslash \{1,2\}$. 
	\end{proof}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%% Problem 50 %%%%%%%%%%	
\subsection*{Problem 50}
For each $n\in \N$, let $P_n$ denote the statement ``$n^2 +5n +1$ is an even integer.''
\begin{enumerate}[label=(\alph*)]
\item Prove that $P_{n+1}$ is true if we assume $P_n$ is true.
	\begin{proof}
        We want to show that $P_n\Longrightarrow P_{n+1}$ such that if:
        \begin{align*}
            n^2+5n+1&=2k, \;k\in \Z, \text{ then, }\\
            (n+1)^2+5(n+1)+1&=2j, \;j\in \Z. 
        \end{align*}
        Let $n^2+5n+1 = 2k$. Consider that:
        \begin{align*}
            ...
            &= 2j, \;j\in \Z
        \end{align*}
        Hence, we have that $P_n\Longrightarrow P_{n+1}$. 
	\end{proof}
			
\item For which $n$ is $P_n$ actually true? What is the moral of this exercise?\vp
	There is no $n$ for which this statement is true. This is because the inductive step does not predicate a base case: it alone can be true without it ever existing in reality. \vp
    The moral is that we need a base case, otherwise we could prove an inductive step that has no applicaibility because there doesn't exist an $n$ to start our induction from. 
